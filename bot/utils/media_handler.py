    ...
    for recipient in recipients:
        ...
        try:
            ...
            if update.message.reply_to_message:
                # Find which message in the recipient's chat corresponds to the message being replied to
                msg_map = await db.get_relayed_message_info_by_relayed_id(
                    chat_id=sender.id, message_id=update.message.reply_to_message.message_id
    ...
    for sender_id, messages in buffer_copy.items():
        ...
        for msg in messages:
            ...
            if is_new_album and current_album_messages:
                all_albums_to_send.append((_create_album_from_messages(current_album_messages), current_album_messages))
                current_album_messages = []
        ...
        for i, batch in enumerate(recipient_batches):
            for recipient in batch:
                for album_input_media, original_msgs in all_albums_to_send:
                    if not album_input_media: continue
                    try:
                        # Log replies correctly
                        reply_to_msg_id = None
                        if original_msgs[0].reply_to_message:
                            msg_map = await db.get_relayed_message_info_by_relayed_id(
                                chat_id=sender_id, message_id=original_msgs[0].reply_to_message.message_id
                        ...
                        # Log for /delete command
                        for j, sent_msg in enumerate(sent_messages):
                            original_msg = original_msgs[j]
                            await db.log_relayed_message(original_msg.message_id, sender_id, {str(recipient['user_id']): sent_msg.message_id})
                    except Forbidden:
                        await db.update_user_status(recipient['user_id'], 'inactive')
                    except Exception as e:

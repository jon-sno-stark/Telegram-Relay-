...
def create_bot_application(bot_token: str) -> Application:
    ...
    # Special handler for deleting messages, must be a reply
    ...
    # Main message handler for relaying (text, media, etc.)
    ...
    job_queue.run_repeating(scheduled_jobs.check_inactive_users, interval=3600, first=60) # Every hour
    job_queue.run_repeating(scheduled_jobs.send_service_message, interval=3600 * 3, first=120) # Every 3 hours
    job_queue.run_repeating(scheduled_jobs.send_daily_summary, interval=3600 * 24, first=180) # Daily
    job_queue.run_repeating(scheduled_jobs.send_weekly_summary, interval=3600 * 24 * 7, first=300) # Weekly
    job_queue.run_repeating(scheduled_jobs.process_media_buffers_job, interval=20, first=20) # Process media buffer every 20s
